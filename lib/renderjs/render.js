...
function changePauseState(pauseState){
...
pauseIndicator.addEventListener('click', () => {
    pauseState = changePauseState(pauseState);
});
...
// Notify user compiler is paused ( webview ). 
const pauseNotifier = document.getElementById('pauseNotifier');
window.addEventListener('message', event=> {
    const message = event.data; 

    if(message.type === 'pauseNotification'){
        pauseNotifier.classList.add('active');
        setTimeout(() => {
            pauseNotifier.classList.remove('active');
        }, 5000);
    }
});

document.getElementById('pauseNotifierCloseButton').addEventListener('click', () => {
    pauseNotifier.classList.remove('active'); 
});
